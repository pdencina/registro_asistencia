...
import { Users, UserCheck, UserX, LogOut, Clock, TrendingUp } from 'lucide-react';
      ...
      <div className="card">
        ...
        <div className="space-y-3">
          {summary.last_records.map(record => (
            <div key={record.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400">{new Date(record.timestamp).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}</p>
              </div>
          ...
          {summary.last_records.length === 0 && (
            <p className="text-center text-gray-400 py-4">Sin registros hoy</p>
          )}
        </div>
      </div>
    </div>
